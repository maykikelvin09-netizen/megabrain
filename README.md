...
```
$ npx @thiagofinch/mega-brain@latest install meu-brain
...
```bash
npx @thiagofinch/mega-brain@latest update
```

Isto vai:

- ✅ Atualizar core, skills, hooks, rules, templates, agents de cargo
- ✅ **Nunca tocar** em: knowledge/, workspace/, .env, .data/, logs/, agents/external/
- ✅ Mostrar exatamente o que será protegido antes de confirmar
- ✅ Pedir confirmação antes de aplicar

| Comando | Quando usar |
|---------|------------|
| `install` | Primeira instalação (pasta vazia) |
| `upgrade` | Ativar Premium (Community → Premium, cria pasta nova) |
| `update` | Atualizar versão sem perder seus dados |
...
```bash
npx @thiagofinch/mega-brain@latest upgrade
```
...
npx @thiagofinch/mega-brain@latest status

# Ver features disponíveis vs bloqueadas
npx @thiagofinch/mega-brain@latest features
```
...
|-------|:-----------:|----------------|
| `OPENAI_API_KEY` | 💡 Recomendada | Transcrição de vídeos e áudios (Whisper). Só necessária se usar `/ingest` com vídeo/áudio. Se já tem transcrições .txt/.docx, pode pular |
...
```bash
# Instalação e Setup
npx @thiagofinch/mega-brain@latest install [nome]      # Assistente interativo de instalação
npx @thiagofinch/mega-brain@latest setup               # Configurar chaves de API
npx @thiagofinch/mega-brain@latest upgrade             # Upgrade Community → Premium
npx @thiagofinch/mega-brain@latest update              # Atualizar sistema sem perder dados

# Licença e Features
npx @thiagofinch/mega-brain@latest status              # Status da licença e edição
npx @thiagofinch/mega-brain@latest features            # Features disponíveis vs bloqueadas
npx @thiagofinch/mega-brain@latest validate <email>    # Revalidar email de membro
...
Nenhuma chave é estritamente obrigatória. A `OPENAI_API_KEY` é recomendada apenas se você precisa transcrever vídeos/áudios automaticamente via Whisper. Se já tem transcrições em .txt ou .docx, pode pular. Se usa Claude Code via Max/Pro, o acesso ao Claude já está incluído. As demais chaves adicionam funcionalidades extras.
